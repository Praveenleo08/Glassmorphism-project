...
function StoreLocator() {
    ...
    return (
        <div className="min-h-screen w-full relative font-sans overflow-x-hidden text-[#5D4037]">
            ...
            {/* Main Content */}
            <div className="relative z-10 w-full min-h-screen pt-44 pb-20 px-4 flex flex-col items-center">

                {/* Header */}
                <div className="text-center mb-12 animate-fade-in-left">
                    <h1 className="text-6xl md:text-7xl font-sans font-bold text-white drop-shadow-lg mb-2 tracking-tight">Locate Us</h1>
                    <p className="text-xl text-white/90 font-medium tracking-wide">(Company owned parlours)</p>
                </div>

                {/* Filters */}
                <div className="flex flex-wrap gap-4 mb-10 w-full max-w-4xl justify-center animate-fade-in-left" style={{ animationDelay: '0.2s' }}>
                    <button className="hidden md:block px-8 py-3 rounded-full bg-[#333] text-white font-bold shadow-xl transform hover:scale-105 transition-all border border-white/20">
                        Coimbatore
                    </button>
                    <button className="px-8 py-3 rounded-full bg-white/20 backdrop-blur-md border border-white/40 shadow-xl text-white font-bold hover:bg-white/30 transition-all transform hover:scale-105">
                        Select City
                    </button>
                    <button className="px-8 py-3 rounded-full bg-white/20 backdrop-blur-md border border-white/40 shadow-xl text-white font-bold hover:bg-white/30 transition-all transform hover:scale-105">
                        Pincode
                ...
                {/* Main Glass Content */}
                <div className="w-full max-w-6xl h-[600px] bg-white/10 backdrop-blur-2xl border border-white/30 rounded-[40px] shadow-[0_30px_60px_rgba(0,0,0,0.5)] flex flex-col md:flex-row overflow-hidden animate-fade-in-left" style={{ animationDelay: '0.4s' }}>

                    {/* Left: Map Placeholder */}
                    <div className="w-full md:w-[60%] h-1/2 md:h-full relative p-4 group">
                        <div className="w-full h-full rounded-[30px] overflow-hidden bg-gray-900/40 relative shadow-inner border border-white/10 transition-all duration-500 group-hover:shadow-[0_0_30px_rgba(255,255,255,0.1)]">
                            <iframe
                                src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d15665.132805218738!2d76.955832!3d11.016844!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3ba859af2f971cb5%3A0x2fc1c817186f121d!2sCoimbatore%2C%20Tamil%20Nadu!5e0!3m2!1sen!2sin!4v1625123456789!5m2!1sen!2sin"
                                width="100%"
                                height="100%"
                            ...
                            <div className="absolute bottom-6 left-6 bg-white/80 backdrop-blur-md px-4 py-2 rounded-lg shadow-lg pointer-events-none">
                                <p className="text-sm font-bold text-gray-800 flex items-center gap-2"><FaMapMarkerAlt className="text-red-600" /> 24 Locations Nearby</p>
                            </div>
                        </div>
                    </div>

                    {/* Right: List */}
                    <div className="w-full md:w-[40%] h-1/2 md:h-full bg-white/5 backdrop-blur-md border-l border-white/10 flex flex-col">
                        ...
                        <div className="flex-1 overflow-y-auto p-4 scrollbar-thin scrollbar-thumb-white/30 scrollbar-track-transparent hover:scrollbar-thumb-white/50 transition-colors">
                            <div className="flex flex-col gap-4">
                                ...
                                {stores.map((store, i) => (
                                    <div key={store.id + '_dup'} className="p-5 rounded-2xl bg-white/10 border border-white/10 hover:bg-white/20 hover:border-white/30 transition-all cursor-pointer group hover:translate-x-2 duration-300 shadow-md opacity-70 hover:opacity-100">
                                        <h3 className="text-lg font-bold text-white mb-2 uppercase tracking-wide group-hover:text-pink-200 transition-colors">{store.name}</h3>
                                        <p className="text-white/80 leading-relaxed font-medium text-sm border-t border-white/10 pt-2 mt-1">
                                            {store.address}
                                        </p>
