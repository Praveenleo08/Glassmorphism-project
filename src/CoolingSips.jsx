...
// Reuse assets from OurProducts or similar approach
import productsBg from './assets/ea23eba1-dac1-4a76-abed-fefa52559845.png'; // Reusing background
import logo from './assets/logo.png';

// Import images from 'Ice Cream Flavour' folder
...
const CoolingSips = () => {
    ...
    const [basePrice] = useState(215);
    const [isAnimating, setIsAnimating] = useState(false); // For animation trigger
    ...
    // Data Configuration - Matching filenames as requested
    const flavors = [
        ...
        { id: 6, name: 'Butterscotch', file: 'buttersctoch.png', image: butterscotch, color: 'bg-yellow-200' }, // Note spelling in filename
    ...
    return (
        <div className="min-h-screen lg:h-screen w-full relative overflow-x-hidden lg:overflow-hidden font-sans text-[#5c3d2e] selection:bg-[#ff80ab] selection:text-white flex flex-col">
            ...
            <div className="fixed inset-0 z-[-1]">
                ...
                <div className="absolute inset-0 bg-gradient-to-br from-purple-900/10 via-pink-500/10 to-blue-600/10 mix-blend-overlay"></div>
                {/* Animated Orbs */}
                <div className="absolute top-10 left-10 w-96 h-96 bg-pink-500/30 rounded-full blur-[120px] animate-pulse"></div>
                <div className="absolute bottom-10 right-10 w-96 h-96 bg-blue-500/30 rounded-full blur-[120px] animate-pulse animation-delay-2000"></div>
            </div>

            {/* Navbar - Compact */}
            <nav className="flex items-center justify-between px-8 py-4 z-50 bg-white/10 backdrop-blur-md border-b border-white/20">
                ...
                <div className="flex items-center gap-4">
                    <Link to="/products">
                        <button className="px-6 py-2 rounded-full bg-white/20 text-white font-bold text-sm hover:bg-white/40 transition-all border border-white/30 backdrop-blur-md shadow-lg">
                            EXIT
                        </button>
                    </Link>
                </div>
            ...
            <main className="flex-1 flex flex-col lg:flex-row items-center justify-center gap-4 lg:gap-12 px-4 lg:px-12 py-4 pb-24 lg:pb-4 lg:h-[calc(100vh-80px)]">
                    ...
                    <div className="relative w-full max-w-[400px] h-[400px] lg:h-full lg:max-h-[600px] rounded-[3rem] bg-white/10 backdrop-blur-2xl border border-white/30 shadow-[0_20px_60px_rgba(0,0,0,0.15)] flex flex-col items-center justify-center p-8 overflow-hidden group mt-12 lg:mt-0">
                        ...
                        <div className="relative z-10 w-full h-full flex items-center justify-center -mt-6">
                            ...
                            <img
                                ...
                                className={`w-[80%] max-h-[70%] object-contain filter drop-shadow-[0_25px_50px_rgba(0,0,0,0.25)] cursor-pointer hover:scale-110 transition-all duration-700 relative z-10 ${isAnimating ? 'animate-pop-in' : 'animate-float'}`}
                                onError={(e) => { e.target.src = 'https://via.placeholder.com/300x400?text=Shake' }}
                            />
                        ...
                        <div className="absolute bottom-6 left-6 right-auto p-4 bg-white/50 backdrop-blur-xl rounded-2xl border border-white/40 shadow-lg flex flex-col gap-2 max-w-[70%]">
                            ...
                            <div className="flex flex-wrap gap-1.5 max-h-[60px] overflow-y-auto custom-scrollbar">
                                {selectedToppings.map(tid => (
                                    <span key={tid} className="px-2 py-0.5 bg-white/60 rounded-lg text-[10px] font-bold text-[#5c3d2e] shadow-sm border border-white/30 whitespace-nowrap">
                ...
                <div className="w-full lg:w-7/12 min-h-[500px] lg:h-full flex flex-col bg-white/20 backdrop-blur-xl rounded-[2.5rem] border border-white/40 shadow-[0_20px_60px_rgba(0,0,0,0.1)] overflow-hidden lg:overflow-hidden">
                    ...
                    <div className="flex items-center p-2 bg-white/20 border-b border-white/20">
                        {['flavor', 'toppings', 'sauce'].map((tab, idx) => (
                            <button
                                ...
                                className={`flex-1 py-4 rounded-xl text-sm md:text-base font-bold uppercase tracking-wider transition-all duration-300 relative overflow-hidden ${activeTab === tab ? 'bg-white shadow-lg text-[#D81B60]' : 'text-gray-600 hover:bg-white/30'}`}
                            >
                                <span className="relative z-10 flex items-center justify-center gap-2">
                                    <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs ${activeTab === tab ? 'bg-[#D81B60] text-white' : 'bg-gray-400/50 text-white'}`}>
                                        {idx + 1}
                                    </span>
                                    {tab}
                                </span>
                    ...
                    {/* Content Area - Scrollable Grid within Panel */}
                    <div className="flex-1 overflow-y-auto custom-scrollbar p-6 bg-gradient-to-b from-transparent to-white/10">

                        {/* FLAVOR TAB */}
                        {activeTab === 'flavor' && (
                            <div className="animate-slide-up">
                                <h3 className="text-2xl font-bold text-[#5c3d2e] mb-6 drop-shadow-sm">Choose Your Base</h3>
                                <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
                                    {flavors.map(flavor => (
                                        <div
                                            key={flavor.id}
                                            onClick={() => handleFlavorSelect(flavor.id)}
                                            className={`group cursor-pointer rounded-2xl p-4 flex flex-col items-center gap-3 transition-all duration-300 border backdrop-blur-sm ${selectedFlavor === flavor.id ? 'bg-white border-pink-500 shadow-[0_10px_20px_rgba(216,27,96,0.2)] scale-105' : 'bg-white/40 border-white/40 hover:bg-white/70'}`}
                                        >
                                            <div className="relative w-20 h-20 transition-transform duration-500 group-hover:scale-110 group-hover:rotate-6">
                                                ...
                                                {selectedFlavor === flavor.id && <div className="absolute -top-2 -right-2 bg-[#D81B60] text-white rounded-full p-1 shadow-md"><FaCheck size={10} /></div>}
                                            </div>
                                            <span className={`text-sm font-bold text-center ${selectedFlavor === flavor.id ? 'text-[#D81B60]' : 'text-[#4E342E]'}`}>{flavor.name}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                                    ...
                                    {toppings.map(topping => {
                                        ...
                                        return (
                                            <div
                                                ...
                                                className={`group cursor-pointer rounded-2xl p-3 flex flex-col items-center gap-2 transition-all duration-300 border backdrop-blur-sm ${isSelected ? 'bg-gradient-to-br from-pink-50 to-white border-pink-500 shadow-lg scale-105' : 'bg-white/40 border-white/40 hover:bg-white/70'}`}
                                            >
                                                <div className="relative w-16 h-16 transition-transform duration-300 group-hover:scale-110">
                                                    <img src={topping.image} alt={topping.name} className="w-full h-full object-contain filter drop-shadow-md" />
                                                    {isSelected && <div className="absolute -top-2 -right-2 bg-[#D81B60] text-white rounded-full p-1 shadow-md"><FaCheck size={10} /></div>}
                                                </div>
                                                <div className="text-center">
                                                    <span className={`block text-xs font-bold ${isSelected ? 'text-[#D81B60]' : 'text-[#4E342E]'}`}>{topping.name}</span>
                        ...
                        {activeTab === 'sauce' && (
                            <div className="animate-slide-up">
                                <h3 className="text-2xl font-bold text-[#5c3d2e] mb-6 drop-shadow-sm">Drizzle Support</h3>
                                <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
                                    {sauces.map(sauce => {
                                        ...
                                        return (
                                            <div
                                                ...
                                                className={`group cursor-pointer rounded-2xl p-4 flex flex-col items-center gap-3 transition-all duration-300 border backdrop-blur-sm ${isSelected ? 'bg-white border-pink-500 shadow-lg scale-105' : 'bg-white/40 border-white/40 hover:bg-white/70'}`}
                                            >
                                                ...
                                                <span className={`text-sm font-bold text-center ${isSelected ? 'text-[#D81B60]' : 'text-[#4E342E]'}`}>{sauce.name}</span>
                                            </div>
                                        );
                                    })}
                                </div>
